...
제공된 노트 내용과 사용자 요청을 기반으로, 내용을 단 두 개의 논리적인 학습 세션(learning path)으로 분할해주세요.
"""),
...
4.  **마무리 유도**: 학생이 개념을 이해한 것 같으면, 자연스럽게 "이제 이 내용을 잘 이해했는지 퀴즈로 확인해볼까요?" 와 같이 다음 단계(퀴즈)를 암시하며 대화를 마무리하세요.
"""),
...
DECIDE_AFTER_INSTRUCTOR_PROMPT = ChatPromptTemplate.from_messages([
    ...
    대화 기록을 보고, 설명이 충분히 이루어져 사이먼AI가 퀴즈로 넘어가려고 한다면 'move_to_quiz'로, 설명이 더 이루어질 가능성이 있거나 설명 중이라면 'continue_explaining'으로 결정하세요.
    만약 사이먼 AI가 사용자에게 '이해되셨나요? 다른 질문은 없을까요?'라는 식으로 되묻는다면 이는 여전히 설명이 진행중인 것이므로 'continue_explaining'으로 결정해야 합니다. 
    그외에 '확인 문제(퀴즈)로 넘어갈까요?라고 명시적으로 말했을 경우에는 'move_to_quiz'로 결정하세요.
    """),
...
5.  **마무리 유도**: 학생이 정답을 맞혔고, "더 풀어볼래요?"라는 질문에 "괜찮아요" 또는 "아니요"라고 답했다면, "좋습니다! 다음 단계로 넘어가길 원하시면 '다음으로'라고 입력해주세요."와 같이 대화를 마무리하세요.
...
DECIDE_AFTER_TESTER_PROMPT = ChatPromptTemplate.from_messages([
    ...
    학생이 정답을 맞혔으며 추가 문제를 원하지 않아 퀴즈를 마쳐도 된다면 'finish_path'로, 
    퀴즈가 계속 진행 중이면 'continue_quiz'로 결정하세요.

    (강조) 학생이 정답을 맞혔더라도, 추가 문제 풀이 질문에 대해 '괜찮아요', '아니요'라고 말하기 전까지는 'continue_quiz'로 판단해야 합니다.
    반대로 학생이 정답을 맞힌 상황이고, 추가 문제 풀이 질문에 '괜찮아요', '아니요'라고 답하거나 '다음으로'라고 응답하면 바로 'finish_path'로 결정해야 합니다.
    """),
    MessagesPlaceholder(variable_name="messages_for_classification")
])
