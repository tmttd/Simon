    ...
    async def post(self, request, *args, **kwargs) -> Response:
        ...
        try:
            if thread_id:
                ...
                thread = await sync_to_async(Thread.objects.create)(user=request.user, title=title)
            ...
            # AI 호출을 비동기로 래핑 - 여기가 핵심!
            ai_response = await sync_to_async(get_ai_response)(user_message, str(thread.id))
